...
export async function GET(request) {
  ...
  const videos = await getVideos("xN2V7TjlvtU");
  ...
  videos.forEach((video) => {
    if(video.id == "xN2V7TjlvtU") {
      console.log(video);
      
      videoData.video.videoId      = video.id;
    }
  });

  return new Response(videoData.video.videoId);
...
export async function getVideos(VIDEO_ID) {
  var fs = require('fs');
